...
⚠️ СТРОГИЕ ПРАВИЛА:
1. Копируй фразы ТОЧНО как в списке — ни одного изменения
2. КАЖДАЯ фраза должна попасть в какой-то кластер
3. НЕ создавай свои фразы, НЕ удаляй фразы, НЕ изменяй текст
...
{{
  "clusters": [
    {{
      ...
      "intent": "commercial",
      "phrases": [
        {{"phrase": "точная копия из списка", "count": число}}
      ]
    }}
  ],
  "minus_words": {{}}
...
✓ Все ли фразы из списка попали в кластеры?
✓ Нет ли дублей в названиях кластеров?
✓ Фразы скопированы ТОЧНО как в списке?
✓ Каждый кластер = уникальный интент?"""
    else:
        prompt = f"""Ты эксперт по SEO-кластеризации для создания структуры сайта.

⚠️ СТРОГИЕ ПРАВИЛА:
1. Копируй фразы ТОЧНО как в списке
2. КАЖДАЯ фраза должна попасть в кластер
3. НЕ создавай/удаляй/изменяй фразы
