...
def clusterize_with_openai(phrases: List[Dict[str, Any]], mode: str = 'context', region_names: List[str] = None, selected_intents: List[str] = None) -> tuple:
    ...
    if mode == 'context':
        prompt = f"""Ты эксперт по контекстной рекламе Яндекс.Директ. 

⚠️ КРИТИЧЕСКИ ВАЖНО:
1. ❌ НЕ ИЗМЕНЯЙ фразы из списка — копируй их ТОЧНО как есть
2. ❌ НЕ ДОБАВЛЯЙ фразы от себя — используй ТОЛЬКО из списка ниже
3. ❌ НЕ УДАЛЯЙ фразы — все фразы должны попасть либо в кластер, либо остаться неотсортированными
4. ✅ ТОЛЬКО создавай названия кластеров и распределяй существующие фразы
...
1. Узкие кластеры (3-15 фраз) — каждая группа максимально однородная
2. Разделяй по: тип товара, характеристики, действия, цена, срочность
3. Название кластера — 2-4 слова из самых частотных фраз группы
4. Используй ТОЛЬКО выбранные интенты: {selected_intents_text}

🚫 НЕ ДОБАВЛЯЙ ФРАЗЫ В minus_words!
Минус-слова НЕ НУЖНЫ. Оставь это поле пустым: {{}}.
Если фраза не целевая — просто не включай её ни в один кластер.

📋 СПИСОК ФРАЗ ({len(phrases[:150])} запросов):
{phrases_text}

⚡ ОТВЕТ В JSON (копируй фразы ТОЧНО):
{{
  "clusters": [
    {{
      "cluster_name": "Название 2-4 слова",
      "intent": "commercial/transactional/informational/navigational",
      "phrases": [
        {{"phrase": "точная копия фразы из списка выше", "count": число_показов_из_списка}}
      ]
    }}
  ],
  "minus_words": {{}}
        ...
        prompt = f"""Ты SEO-эксперт. 

⚠️ КРИТИЧЕСКИ ВАЖНО:
1. ❌ НЕ ИЗМЕНЯЙ фразы из списка — копируй их ТОЧНО как есть
2. ❌ НЕ ДОБАВЛЯЙ фразы от себя — используй ТОЛЬКО из списка ниже
3. ❌ НЕ УДАЛЯЙ фразы — все должны попасть либо в кластер, либо остаться неотсортированными
4. ✅ ТОЛЬКО создавай названия кластеров и распределяй существующие фразы
...
🎯 ПРАВИЛА КЛАСТЕРИЗАЦИИ:
1. Средние/широкие кластеры (10-40 фраз) — по общему смыслу и intent
2. Группируй по: общая тема, похожий интент, релевантность одной странице
3. Название кластера — 2-5 слов, отражает суть группы
4. Используй ТОЛЬКО выбранные интенты: {selected_intents_text}
