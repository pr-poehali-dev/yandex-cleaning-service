...
3. ТОЛЬКО группировка существующих фраз по их смыслу

📊 Правила кластеризации:
- Узкие кластеры (3-10 фраз) для точного таргетинга
- Разделяй по ЛЮБЫМ конкретным признакам в фразах: характеристики, типы, размеры, цвета, бренды, услуги
- Ищи отличия между фразами и группируй похожие отдельно
- Название кластера = 2-3 ключевых слова из самых частотных фраз кластера
- Каждая фраза ТОЛЬКО в одном кластере

🗑️ Минус-слова:
Определи нецелевые запросы и раздели на категории:
- free: бесплатно, даром, халява
- info: как сделать, инструкция, своими руками, отзывы, форум
- competitors: названия конкурентов, маркетплейсы
- irrelevant: вакансии работа, курсы обучение, несвязанные темы
...
- Анализируй фразы и группируй по интенту и смыслу пользователя
- Разделяй по конкретным признакам если они есть в фразах
- Название = 2-4 ключевых слова из самых частотных фраз кластера
- Каждая фраза ТОЛЬКО в одном кластере
